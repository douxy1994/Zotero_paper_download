...
登录后，工具每 4 秒检测一次本篇 PDF；验证到 PDF 字节后保存会话并自动关闭。若站点结构未被识别，点击网页右下角“已登录，保存并重试”。点击“取消登录”则关闭专用窗口并进入保底。手工关闭窗口视为取消。
...
进度窗改用独立 XHTML 页面，固定最小可用尺寸，只初始化一次，避免 about:blank 导航和 HTML/XUL 混用导致内容消失。关闭进度窗会取消任务。登录助手只关闭它自行创建的窗口；浏览器页面每 4 秒探测本篇 DOI 的 PDF 链接，取得 PDF 文件后保存会话并关闭。页面提供“已登录，保存并重试”和“取消登录”按钮；仅凭 Cookie 数量或“Log in”文字不宣称登录成功。登录后下载文件仍须通过 PDF 身份验证。

## 0.8.3 Elsevier 登录修复

优先使用题录中完整的出版社页面 URL。Elsevier 的 PII PDF 链接由完整的 17 位标识匹配，不依赖 URL 包含 DOI；SSO 重定向中断页面加载时保持登录窗口等待。登录助手失败显示实际阶段，不再被 Paper-fetch 缺少 API Key 的早期诊断覆盖。Cookie 登录与 Elsevier API Key 是独立通道，前者无需通过后者判定成功。真实全文仍须通过 PDF 身份验证。

## 0.8.4 登录按钮响应

完成/取消按钮改用 DOM 属性传递状态，兼容浏览器脚本隔离。点击后立即显示“已收到”，并禁用重复点击。每轮最多一个 3 秒 PDF 请求，避免连续请求阻塞按钮响应。页面使用完整 DOI/PII 验证候选，最终 PDF 身份校验保留。该修复适用于更新后新打开的登录窗口。

## 0.8.5 同一登录浏览器交付 PDF

Elsevier 不再调用上游截断 PII 的 get 浏览器路径。专用浏览器使用独立持久化 Profile，在有权限的文章页点击实际 View PDF，接收浏览器下载事件，保存 PDF 后关闭。点击“已登录”会在同一窗口继续下载，不会关闭后重新调用 ScanSci DOI 解析器；后者是此前重复弹窗与截断 PII 的来源。仅本插件浏览器设为直接下载 PDF，不改变日常 Chrome 设置。
